...
d3.csv('dataset-and-analysis/gender-by-subject.csv', function (data) {
    var level = "All";
    var country = "All";
    var sex = "Female";
    ...
    document.addEventListener("reloadChart3Event", function () {
        var levelDropdown = document.getElementById('level-3');
    ...
    var sexByProviderVis = d3.select('#vis-3')
    ...
    var yAxisTopic = svg.append("g")
        .append("text")
        .attr("class", "vis3-y-axis-topic")

    yAxisTopic.append("tspan")
        .attr("id", "vis3-y-axis-topic-1")
        .attr("x", 0)
        .attr("y", 0)
        .attr("dy", -24)
    ...
    function update() {
        var convertedData = processSexBySubjectRankingData(data, level, country, sex);
        ...
        var maxDomainY = d3.max(convertedData, function (d) {
            return d[sex + " (%)"];
        });
        ...
        new_lines.merge(lines)
            ...
            .attr("height", function (d, i) {
                return height - y_Scale(d[sex + ' (%)']);
            })
            .attr("y", function (d, i) {
                return y_Scale(d[sex + ' (%)']);
            })
...
function processSexBySubjectRankingData(rawData, level, dest, sex) {
    if (sex == 'Female') {
        rawData.sort((a, b) => (a['Female (%)'] < b['Female (%)']) ? 1 : -1);
    } else if (sex == 'Male') {
        rawData.sort((a, b) => (a['Male (%)'] < b['Male (%)']) ? 1 : -1);
    } else if (sex == 'Other') {
        rawData.sort((a, b) => (a['Other (%)'] < b['Other (%)']) ? 1 : -1);
    }

    selectedData = rawData.filter(function (row) {
        return row['Level of study'] == level && row['Country of HE provider'] == dest && parseFloat(row[sex + ' (%)']) != 0;
        //Filter rows with value 0 out to prevent domain on X axis appear even though the values are 0.
    });
